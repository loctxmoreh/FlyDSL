...
|---|---|---|---|---|
| `gfx90a` | MI250 / MI250X | 64 | MFMA | **Experimental — `dev/gfx90a` branch, NOT in the official support matrix (CDNA3+).** CDNA2. Works: f16/bf16/f32 + **int8** MFMA GEMM (preshuffle, K=16 i8 on gfx90a), **f16 split-K HGEMM** (glc-scope coherence), f16/bf16 MoE with f16 output, norm/softmax/RoPE, tiled copy/MMA. Fail-fast / unsupported: FP8 & FP4/MX (no hardware), K=32/16 f16/bf16 MFMA + 128b `buffer_load_dwordx4_lds` (CDNA4), and anything needing a packed bf16 atomic (bf16 split-K SPLIT_K>1, bf16-output MoE). `is_rdna_arch()` False, wave64. Status & triage: [`docs/gfx90a_triage.md`](docs/gfx90a_triage.md); follow-ups: [`TODO.md`](TODO.md). |
